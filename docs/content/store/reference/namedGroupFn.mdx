...
const group = namedGroup({launch: emitter<void>(), data: state<number>(0)}, ({launch}) => {
  launch
    // Performs an effect each time launch fires (void payload)
    .effect(
      switchMap(() => productsService.get('api/products')),
    );
});
...
// Accesses the 'launch' emitter through the group and triggers loading
group.launch.emit();
