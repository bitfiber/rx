...
[reactive state](/store/reference/stateFn) to specific parts of external storages, ensuring that
state data remains synchronized and up to date. This approach streamlines the management of data
consistency and alignment across your application.
...
// Create a state
const userPreferences = state({theme: 'dark', notifications: true}, s =>
  // Connect the state to the user preferences source
  s.connect(localStoragePart('userPreferences')));
...
interface. This allows your custom data source to integrate seamlessly with [states](/store/reference/stateFn)
