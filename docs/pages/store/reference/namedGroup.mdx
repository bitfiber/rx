...
// Define a named group that contains emitters, states, or other groups
...
// Initialize the group and all items it contains
myGroup.initialize();

// Emit to trigger the 'launch' emitter
myGroup.launch.emit();

// Complete the group, finalizing all its items
myGroup.complete();
```
